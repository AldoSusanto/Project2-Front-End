...
class Play extends Component {
            ...
            <div className="button-container d-flex justify-content-center justify-content-md-start">
              <button
                ...
                className={classNames("", {
                  disable: this.state.currentSelectedTags.length <= 0,
                })}
              >
                NEXT
              </button>
  ...
  buttonClick(event) {
    ...
    switch (event.target.id) {
      ...
      case "next-button":
        ...
        var tagArray = this.state.collectedTags.concat(
          this.state.currentSelectedTags
        );
      ...
      default:
        console.error(
          "A button is clicked, but the ID is unidentified " + event.target.id
        );
        break;
    }
  }
    ...
    if (!currentQuestion.isMultiple) {
      for (var i = 0; i < questions.length; i = i + 2) {
        if (i + 1 < questions.length) {
          ...
          let q1Selected = this.state.currentSelectedChoices.includes(
            currentQuestion.questionLabel + q1
          );
          let q2Selected = this.state.currentSelectedChoices.includes(
            currentQuestion.questionLabel + q2
          );
          ...
          choices.push(
            <div key={i} className="options-container">
              <p
                key={questions[i]}
                onClick={(e) => this.userAnswered(e, q1)}
                className={q1Selected ? "option selected" : "option"}
              >
                {questions[i]}
              </p>
              <p
                key={questions[i + 1]}
                onClick={(e) => this.userAnswered(e, q2)}
                className={q2Selected ? "option selected" : "option"}
              >
                {questions[i + 1]}
              </p>
            </div>
        ...
        if (i + 1 < questions.length) {
          ...
          choices.push(
            <div key={i} className="options-container">
              <p
                key={questions[i]}
                onClick={(e) => this.userAnsweredMultiple(e, q1)}
                className={
                  q1Selected ? "option-multiple selected" : "option-multiple"
                }
              >
                {questions[i]}
              </p>
              <p
                key={questions[i + 1]}
                onClick={(e) => this.userAnsweredMultiple(e, q2)}
                className={
                  q2Selected ? "option-multiple selected" : "option-multiple"
                }
              >
                {questions[i + 1]}
              </p>
  ...
  userAnswered(event, tagIndex) {
    ...
    var indexAndQuestionLabel =
      this.state.currentQuestion.questionLabel + tagIndex;
    this.setState({
